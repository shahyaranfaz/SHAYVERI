...
CHUNKS="${CHUNKS:-50}" # 0 means run forever.
...
while (( CHUNKS == 0 || made < CHUNKS )); do
  chunk_id="$(printf '%s_%06d_%s' "$WORKER_ID" "$chunk_index" "$(date +%s)")"

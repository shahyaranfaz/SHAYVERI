...
fn main() {
    ...
    let train_id = env::var("TRAIN_ID").unwrap_or_else(|_| "sf1b_kb16_512".to_string());
    let out_dir = env::var("OUT_DIR").unwrap_or_else(|_| "checkpoints".to_string());
    let resume = env::var("RESUME").unwrap_or_default();

    let initial_lr: f32 = env_or("LR", 0.0005);
    let final_lr: f32 = env_or("FINAL_LR", initial_lr);
    let superbatches: usize = env_or("EPOCHS", 18);
    let batch_size: usize = env_or("BATCH_SIZE", 16_384);
    let batches_per_superbatch: usize = env_or("BATCHES_PER_SUPERBATCH", 3052);
    let wdl_proportion: f32 = env_or("WDL", 0.30);
    let scale: f32 = env_or("SCALE", 400.0);
    let save_rate: usize = env_or("SAVE_EPOCHS", 3);

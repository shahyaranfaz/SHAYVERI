...
# Master-side consumer for SHAYVERI NNUE v2.11 KB16 data.
...
NFS_ROOT="${NFS_ROOT:-$HOME/nnue_v2_11}"
...
BULLET_EXAMPLE="${BULLET_EXAMPLE:-shayveri_kb16}"
...
if [[ "$BULLET_TRAIN_CMD" == *"--example $BULLET_EXAMPLE"* ]]; then
  if [[ ! -f "$BULLET_DIR/examples/$BULLET_EXAMPLE.rs" ]]; then
    ...
    echo "copy $BULLET_EXAMPLE.rs there, or override BULLET_TRAIN_CMD" >&2
    exit 1
  fi
fi
...
process_chunk() {
  ...
  for plain in "${plain_files[@]}"; do
    ...
    rm -f "$bullet"
    "$BULLET_UTILS" validate --input "$shuffled" || return 1
  done

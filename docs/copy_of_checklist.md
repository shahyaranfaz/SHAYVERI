...
`v2.9.1` as an example.

## 1. Finalize the engine

- [x] Choose the version according to [`VERSIONING.md`](VERSIONING.md).
- [x] Add the new version's number to the GitHub actions workflow.
- [x] Freeze the release scope, production defaults, and default NNUE.
- [x] Remove temporary gates and unfinished experimental behaviour.
- [x] Confirm tuning-only options are absent from the production UCI handshake.
...
```sh
cd ~/elo_pin
PIN_ROOT="$PWD" RELEASE_ID=v2.9.1 \
...
```sh
git tag -a v2.9.1 -m "SHAYVERI v2.9.1"

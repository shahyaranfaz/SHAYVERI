...
HARDCODE_OPENING_FILE = os.environ.get(
    "OPENING_FILE",
    "../books/final_search_mix_shuf.epd",
)
...
def get_match_commands(engine_file, test_options, base_options,
    ...
    if match_manager == 'fastchess':
        ...
        raise ValueError(f'match manager {match_manager} is not supported')
    ...
    if match_manager == 'fastchess':
        ...
        command += f' -pgnout file={pgn_output} min=true'
        command += f' -scoreinterval {games}'
        command += ' -ratinginterval 0'

        if move_time is not None:
            command += f' -each st={move_time} timemargin={timemargin}'
        elif nodes is not None:
            command += f' -each tc=inf nodes={nodes} timemargin={timemargin}'
        else:
            if base_time_sec is not None and inc_time_sec is not None and depth is not None:
                command += f' -each tc={base_time_sec}+{inc_time_sec} depth={depth} timemargin={timemargin}'
            elif base_time_sec is not None and inc_time_sec is not None:
                command += f' -each tc={base_time_sec}+{inc_time_sec} timemargin={timemargin}'
            elif base_time_sec is not None:
                command += f' -each tc={base_time_sec} timemargin={timemargin}'
            elif inc_time_sec is not None and depth is not None:
                command += f' -each tc=0+{inc_time_sec} depth={depth} timemargin={timemargin}'
            elif inc_time_sec is not None:
                command += f' -each tc=0+{inc_time_sec} timemargin={timemargin}'
...
def engine_match(engine_file, test_options, base_options, opening_file,
                 ...
                 match_manager='fastchess', match_manager_path=None,
    ...
    if os_name.lower() == 'windows':
        process = Popen(str(tour_manager) + command, stdout=PIPE, stderr=STDOUT, text=True)
    else:
        process = Popen(shlex.split(str(tour_manager) + command), stdout=PIPE, stderr=STDOUT, text=True)
    ...
    for eline in iter(process.stdout.readline, ''):
        ...
        if any(token in lower for token in (
            ...
            raise Exception(f'match failed: {line}')
    ...
    if return_code != 0:
        raise Exception(
            f'match manager returned non-zero exit code: {return_code}\n' +
            "\n".join(tail)
        )
    ...
    def run_job(job_file: Path) -> Path:
        ...
        try:
            result = engine_match(
                ...
                variant=job["variant"],
                fastchess_debug=job["fastchess_debug"],
                fastchess_wait=job["fastchess_wait"],
        ...
        for _ in range(remaining_budget):
            ...
            while len(in_flight) < max_outstanding:
                ...
                job = {
                    ...
                    "fastchess_debug": args.fastchess_debug,
...
def main():
    ...
    parser.add_argument('--match-manager', required=False, type=str, default='fastchess')
    ...
    parser.add_argument('--fastchess-debug', action='store_true')
    parser.add_argument('--fastchess-wait', required=False, type=int, default=5000)

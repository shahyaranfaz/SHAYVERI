...
There are two available evaluation paths: `NNUE` and `HCE-classical`, which can
be chosen between with the `UseNNUE` UCI option.

## Handcrafted evaluation

- Piece-square tables with tapered middlegame/endgame interpolation
- Pawn structure: passed pawns, isolated pawns, doubled/backward pawns, pawn
  chains, pawn storms, pawn islands
- King safety: pawn shield integrity, open files toward king, enemy pressure in
  king zone, escape squares
...
Built from 2600+ rated games sourced from The Week in Chess (TWIC). The pipeline
keeps 24-ply lines from standard-chess games where both players are rated at
...
instruction-level profile. For comparable results, use an idle machine and
keep the compiler, binary settings, CPU pinning, and bench signature fixed.

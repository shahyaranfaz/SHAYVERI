...
MoveList generate_pseudo_legal_moves(Board& b) {
    ...
    while (knights) {
        Square from = __builtin_ctzll(knights);
        knights &= knights - 1;
        U64 mask = knight_attacks(from) & ~curr_occupied;
        push_moves_from_mask(moves, from, mask);
    }
    ...
    while (bishops) {
        Square from = __builtin_ctzll(bishops);
        bishops &= bishops - 1;
        U64 mask = bishop_attacks(from, b.occupied) & ~curr_occupied;
        push_moves_from_mask(moves, from, mask);
    }
    ...
    while (rooks) {
        Square from = __builtin_ctzll(rooks);
        rooks &= rooks - 1;
        U64 mask = rook_attacks(from, b.occupied) & ~curr_occupied;
        push_moves_from_mask(moves, from, mask);
    }
    ...
    while (queens) {
        Square from = __builtin_ctzll(queens);
        queens &= queens - 1;
        U64 mask = queen_attacks(from, b.occupied) & ~curr_occupied;
        push_moves_from_mask(moves, from, mask);
    }

    // king + castling
    Square ksq = king_square(b, curr);
    U64 king_mask = king_attacks(ksq) & ~curr_occupied;
    push_moves_from_mask(moves, ksq, king_mask);
    ...
    static constexpr U64 WK_CASTLE_EMPTY = bb_square(make_square(FILE_F, RANK_1)) | bb_square(make_square(FILE_G, RANK_1));
    static constexpr U64 WQ_CASTLE_EMPTY = bb_square(make_square(FILE_B, RANK_1)) | bb_square(make_square(FILE_C, RANK_1)) | bb_square(make_square(FILE_D, RANK_1));
    static constexpr U64 BK_CASTLE_EMPTY = bb_square(make_square(FILE_F, RANK_8)) | bb_square(make_square(FILE_G, RANK_8));
    static constexpr U64 BQ_CASTLE_EMPTY = bb_square(make_square(FILE_B, RANK_8)) | bb_square(make_square(FILE_C, RANK_8)) | bb_square(make_square(FILE_D, RANK_8));

    if (curr == WHITE) {
        if ((b.castling & WHITE_KINGSIDE)
            && !(b.occupied & WK_CASTLE_EMPTY)
            && (b.bit_boards[WR] & bb_square(make_square(FILE_H, RANK_1)))
            && !is_square_attacked(b, E1, other)
            && !is_square_attacked(b, make_square(FILE_F, RANK_1), other)
            && !is_square_attacked(b, G1, other))
        ...
        if ((b.castling & BLACK_KINGSIDE)
            && !(b.occupied & BK_CASTLE_EMPTY)
            && (b.bit_boards[BR] & bb_square(make_square(FILE_H, RANK_8)))
            && !is_square_attacked(b, E8, other)
            && !is_square_attacked(b, make_square(FILE_F, RANK_8), other)
            && !is_square_attacked(b, G8, other))
